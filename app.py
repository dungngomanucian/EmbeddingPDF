...
def upload_page():
    ...
    if request.method == "POST":
        ...
        if not file or not file.filename:
            ...
            try:
                success, msg = process_upload(file)
...
def search():
    ...
    try:
        dists, idxs, id_map = faiss_manager.search(q, top_k=Config.TOP_K_SEARCH)
        
        if not id_map or idxs.size == 0 or idxs[0].size == 0:
             return jsonify({"query": q, "results": [], "type": search_type})

        valid_matches = []
        for i, dist in zip(idxs, dists):
            if i > -1 and dist >= Config.SIMILARITY_THRESHOLD:
                if i in id_map: 
                    valid_matches.append((id_map[i], float(dist)))
                
        if not valid_matches:
            return jsonify({"query": q, "results": [], "type": search_type})
            
        top_ids = [mid for mid, _ in valid_matches]
        scores = {mid: s for mid, s in valid_matches}
        
        res = supabase.table('documents').select('id, content, metadata').in_('id', top_ids).execute()
        
        sorted_docs = sorted(res.data, key=lambda x: top_ids.index(x['id']))
        results = []
        context_for_ai = []
        ...
        for doc in sorted_docs:
            ...
            if search_type == 'image':
                ...
                context_for_ai.append(clean_content)
